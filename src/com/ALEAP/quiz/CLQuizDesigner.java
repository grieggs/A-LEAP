...
public class CLQuizDesigner {
	public static void main(String args[]){
			...
			if(input.toUpperCase().equals("N")){
				...
				while (!tDone){
					...
					if(input1.toUpperCase().equals("MC")){
						...
						int diff = in.nextInt();
						in.nextLine();
						newQuiz.getTopic(topicId).addQuestion(new MultipleChoiceQuestion(questId++, diff, question, answers[0], answers,0,0));
					}
						...
						newQuiz.getTopic(topicId).addQuestion(
								new Question(questId++, dif, quest, answer, 0, 0));
					}
					else{
						tDone = true;
					}
		...
		File x = new File("H:\\xxxx\\quiz.ser");
		QuizIO.writeQuiz(x, newQuiz);
